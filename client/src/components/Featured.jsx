...
function Featured() {
    ...
    const nextSlide = () => {
        setSlide((prevSlide) => (prevSlide === featuredItems.length - 1 ? 0 : prevSlide + 1));
    };

    const prevSlide = () => {
        setSlide((prevSlide) => (prevSlide === 0 ? featuredItems.length - 1 : prevSlide - 1));
    };

    // Function to handle automatic slide transition
    const autoTransition = () => {
        setAutoSlide((prevAutoSlide) => (prevAutoSlide === featuredItems.length - 1 ? 0 : prevAutoSlide + 1));
    };

    // Set up automatic slide transition interval
    useEffect(() => {
    ...
    return (
        <div className="relative flex items-center flex-col">
            <div className="w-full h-64 flex overflow-hidden relative rounded-lg items-center">
                {/* Carousel Items */}
                {featuredItems.map((item, index) => (
                    <button
                        key={index}
                        className={`h-full w-full absolute block transition-opacity ${slide === index ? "z-10" : "opacity-0"} ${item.color}`}
                    >
                        <Link to={item.link} state={item}>
                            <img
                                src={item.src}
                                className="h-full absolute block -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2"
                                alt={item.title}
                            />
                        </Link>
                    </button>
                ...
                <div className="flex absolute bottom-0 left-1/2 transform -translate-x-1/2 mb-2 z-20">
                    {featuredItems.map((_, index) => (
                        <motion.button
                            ...
                            className={`w-4 h-4 mx-2 rounded-full cursor-pointer filter drop-shadow-md ${slide === index ? "bg-peach border-solid border-2 border-white" : "bg-gray-300"}`}
                        ></motion.button>
                    ))}
                </div>
            </div>
            <motion.button whileHover={{ scale: 1.08, y: "-50%" }} initial={{y: "-50%"}} whileTap={{ scale: 0.9 }} className="absolute left-0 top-1/2 z-20">
                <BsArrowLeftCircleFill onClick={prevSlide} className="text-white text-3xl w-14 filter drop-shadow-md cursor-pointer" />
            </motion.button>
            <motion.button whileHover={{ scale: 1.08, y: "-50%" }} initial={{y: "-50%"}} whileTap={{ scale: 0.9 }} className="absolute right-0 top-1/2 z-20">
                <BsArrowRightCircleFill onClick={nextSlide} className="text-white text-3xl w-14 filter drop-shadow-md cursor-pointer" />
            </motion.button>
